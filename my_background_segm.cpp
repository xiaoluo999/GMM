	...
	double varThreshold, double noiseSigma)
{
	...
	for (y = 0; y < rows; y++)
	{
		...
		if (alpha > 0)//如果学习速率为0，则退化为背景相减
		{
			...
			{
				float wsum = 0;
				float pix = src[x];//每个像素的灰度
				int kHit = -1, kForeground = -1;//是否属于模型，是否属于前景

				for (k = 0; k < K; k++)//每个高斯模型
				{
					float w = mptr[k].weight;//当前模型的权值，初始化为0
					wsum += w;//权值累加
					if (w < FLT_EPSILON)
						break;
					...
					//当前像素不属于任何一个模型
				{
					...
					mptr[k].var = var0;	//初始化方差，defaultNoiseSigma*defaultNoiseSigma * 4=15*15*4
					mptr[k].sortKey = sk0;//初始化优先级
				}
				else
				for (; k < K; k++)
